...
public class AutoGeneratedClassHelper {
    ...
    public static FunctionDef getterForPrimitive(Class<?> clazz) {
        FunctionDef def = new FunctionDef();
        def.setName("Get" + " " + clazz.getSimpleName());
        def.setQualifiedName("GENERATED:GETTER:" + clazz.getName());
        def.getInputParamDefs().add(
        ...
        def.getOutputParamDefs().add(
                new ParamDef(
                        "Var",
                        MetaHolder.PRIMITIVE_DEFINITION.get(clazz.getName())
                )
        );
        def.setCategory("变量相关|Getter函数|" + generatePrefixCategory(clazz) + clazz.getSimpleName());
        def.setDescription("从上下文环境中根据Name获取变量");
        def.setDisplayName("Get " + clazz.getSimpleName());
        def.setExecutable(false);
        def.setSignature(clazz.getName() + ":GETTER()");
    ...
    public static FunctionDef setterForPrimitive(Class<?> clazz) {
        FunctionDef def = new FunctionDef();
        def.setName("Set" + " " + clazz.getSimpleName());
        def.setQualifiedName("GENERATED:SETTER:" + clazz.getName());
        def.getInputParamDefs().add(
        ...
        def.setCategory("变量相关|Setter函数|" + generatePrefixCategory(clazz) + clazz.getSimpleName());
        def.setDescription("从上下文环境中根据Name设置变量");
        def.setDisplayName("Set " + clazz.getSimpleName());
        def.setExecutable(true);
        def.setSignature(clazz.getName() + ":SETTER(INPUT<String>Name,INPUT<" + clazz.getSimpleName() + ">Var)");
    ...
    private static String generatePrefixCategory(Class<?> clazz) {
    ...
    public static FunctionDef mybatisPlusSelectById(Class<?> clazz) {
        FunctionDef def = new FunctionDef();
        def.setName("MybatisPlusSelectById" + " " + clazz.getSimpleName());
        def.setQualifiedName("GENERATED:MYBATISPLUSSELECTBYID:" + clazz.getName());
        def.getInputParamDefs().add(
                new ParamDef(
                        "Id",
                        MetaHolder.PRIMITIVE_DEFINITION.get("java.lang.String")
        ...
        def.setCategory("用户自定义Class|" + clazz.getSimpleName() + "|数据库操作|根据主键查询");
        def.setDescription("根据主键查询SelectById");
        def.setDisplayName("MybatisPlusSelectById " + clazz.getSimpleName());
        def.setExecutable(false);
        def.setSignature(clazz.getName() + ":MYBATISPLUSSELECTBYID()");
        return def;
    }
}
